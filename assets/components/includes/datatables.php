...
if (isset($_GET['helpdesks1'])) {
    $table = "(SELECT h.id, h.date_requested, h.request_number, rt.request_type, c.category, sc.sub_category, s.id as s_id, s.status, s.status_desc, s.color, CONCAT(
    ...
    LEFT JOIN helpdesks_statuses s ON h.status_id = s.id) gethelpdesks";

    $columns = array(
        array('db' => 'request_number', 'dt' => 0),
        array('db' => 'requestor', 'dt' => 1),
        ...
        array(
            ...
            'formatter' => function ($data, $row) {
                $buttons = '<span class="text-nowrap">' .
                    ...
                    ($row['status'] == 'Completed' ? '<button onclick="return useraction(\'view\', ' . $row['id'] . ')" class="btn btn-primary btn-sm mx-1"><i class="bi bi-eye"></i></button><button onclick="return useraction(\'csf\', ' . $row['id'] . ')" class="btn btn-success btn-sm mx-1"><i class="bi bi-list-check"></i></button>' : '') .
                    '</span>';
                return $buttons;
            }
        ),
...
if (isset($_GET['helpdesks4'])) {
    ...
    LEFT JOIN categories c ON h.category_id = c.id
    LEFT JOIN sub_categories sc ON h.sub_category_id = sc.id
    LEFT JOIN helpdesks_statuses s ON h.status_id = s.id WHERE h.requested_by=" . $_SESSION['id'] . ") gethelpdesks";
